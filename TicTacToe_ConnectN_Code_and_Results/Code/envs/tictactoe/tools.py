...
from envs.tictactoe.env import *

tool_names = ["CalculateScores", "GetScores"]
...
class CalculateScores(BaseModel):
    ...
    def execute(self, game, working_memory):
        ...
        if len(working_memory.depth_scores) == 10:
            return "Call function GetScores now! Starting from depth 0 and going down until reaching an end game situation! No need to call CalculateScores again."
        ...
        return "Let's start playing the game. Call function GetScores now!"
...
class GetScores(BaseModel):
    ...
    def execute(self, game, working_memory):
        ...
        if action_score_tuples:
            act = f"Operation: Called function GetScores to retrieve the scores for all actions at the current state and depth {self.depth}."
            obs = (
                f"The list of scores for all available actions presented as (action,score) pairs is: "
                f"{[(move, score) for move, score in action_score_tuples]}. "
                "I should absolutely choose an action and return a response with this format 'I am player 'X' and I am a maximizing player so I choose action 'n' and set thought.exit to True.' "
                "After this you should call GetScores for the next depth and change the player strategy from maximizing to minimizing or vice-versa."
            )
            return act, obs 
        else:            
            return f"Operation: Called function GetScores to retrieve the scores for all actions at the current state and depth {self.depth}.", "Observation: No valid moves available."
